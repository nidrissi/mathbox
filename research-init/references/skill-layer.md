...
Create no repository-local skills. The user-wide canonical suite supplies the
common workflows; the repository supplies local data and rules.

## Vendor exact copies only when

- collaborators need the same workflow from version control;
- a remote/cloud environment cannot access user home skills; or
- an institutional policy requires repository-contained configuration.

Record suite version and checksum. Do not edit vendored canonical skill bodies.

## Create a new project skill only when all are true

1. The procedure recurs.
2. The procedure itself is unique to this project.
3. It cannot be represented by a global skill plus `AGENTS.md`, conventions,
   verification commands, templates, or scripts.
...
Same-name wrappers around personal skills are prohibited because tool
